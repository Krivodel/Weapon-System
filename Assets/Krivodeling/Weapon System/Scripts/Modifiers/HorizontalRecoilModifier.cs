...
{
    public sealed class HorizontalRecoilModifier : WeaponModifier
    {
        ...
        protected override void OnApply(in IWeaponDataReadOnly original, WeaponData modifiable)
        {
            ...
            if (Mode == ModifierMode.Percentage)
                change = original.Recoil.HorizontalPattern.RecoilMultiplier.TakePercent(Value);
            else
                change = Value;

            change *= Mathf.Sign(Value);

            modifiable.Recoil.HorizontalPattern.RecoilMultiplier += change;
        }
